...
import depositNftPresigned from "../app/src/neoSwap.module.v4.2/functions.neoswap/createInstruction/deposit.nft.presigned.neoswap.ci";
...
describe("DataRemovedFronUserPda", () => {
    ...
    it("presigned good user owns deposit wrong nft", async () => {
        let transactionHashs: string[] = [];
        // for await (const userKeypair of userKeypairsNormal) {
        const swapData = await getSwapDataFromPDA({
            provider: program.provider as anchor.AnchorProvider,
        ...
        const { instruction: depositUnauthorizedInstruction } = await depositNftPresigned({
            program,
            user: userKeypairsPresigned[0].keypair.publicKey,
            signer: signer.keypair.publicKey,
            swapDataAccount: pda,
            ataList: [],
            mint: unauthorizedKeypair.tokens[0].mint,
        ...
        try {
            ...
            console.log("Claim & close  :", error);
            assert.ok(String(error).includes(`ProgramFailedToComplete`), true);
        }
    });
    it("bad user not owns good nft mint", async () => {
        let transactionHashs: string[] = [];
        ...
        const { instruction: depositUnauthorizedInstruction } = await depositNftPresigned({
            program,
            user: unauthorizedKeypair.keypair.publicKey,
            signer: signer.keypair.publicKey,
            swapDataAccount: pda,
            ataList: [],
    ...
    it("presigned bad user not owns good nft ata", async () => {
        let transactionHashs: string[] = [];
        // for await (const userKeypair of userKeypairsNormal) {
        const swapData = await getSwapDataFromPDA({
            provider: program.provider as anchor.AnchorProvider,
        ...
        console.log("mint", userKeypairsPresigned[0].tokens[0].mint.toBase58());
        let { mintAta, transaction } = await findOrCreateAta(
            program,
            userKeypairsPresigned[0].keypair.publicKey,
            userKeypairsPresigned[0].tokens[0].mint,
        ...
        const [userPda, userBump] = PublicKey.findProgramAddressSync(
            [unauthorizedKeypair.keypair.publicKey.toBytes()],
            program.programId
        );

        const depositIx = await program.methods
            .depositNftPresigned(swapDataAccount_seed, swapDataAccount_bump, userBump)
            .accounts({
                systemProgram: SystemProgram.programId,
                tokenProgram: TOKEN_PROGRAM_ID,
                swapDataAccount: pda,
        ...
        try {
            ...
            console.log("Claim & close  :", error);
            assert.ok(String(error).includes(`"Custom":6030`), true);
        }
    });
});
